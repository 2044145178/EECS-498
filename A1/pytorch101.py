...
def multiples_of_ten(start, stop):
    ...
    #############################################################################
    # Replace "pass" statement with your code
    if start // 10 == stop // 10 and start % 10 != 0:
        x = torch.empty(0, dtype=torch.float64)
    else:
        if start % 10 == 0:
            x = torch.arange(start // 10 * 10, stop + 1, 10, dtype=torch.float64)
        else:
            # 123
            x = torch.arange((start // 10 + 1) * 10, stop + 1, 10, dtype=torch.float64)
    #############################################################################
    #                            END OF YOUR CODE                               #
    #############################################################################
    return x
...
def reverse_rows(x):
    ...
    #############################################################################
    # Replace "pass" statement with your code
    idx = torch.flip(torch.arange(x.shape[0]), (0,))
...
def batched_matrix_multiply(x, y, use_loop=True):
    ...
    if use_loop:
        ...
        for i in range(x.shape[0]):
            # if i == 0:
            #     z = x[i].mm(y[i]).view(1, x.shape[1], -1)
            # else:
            #     torch
            # z = torch.stack((z, x[i].mm(y[i]).view(1, x.shape[1], -1)))
            temp.append(x[i].mm(y[i]))
        z = torch.stack(temp)
    else:
        z = torch.bmm(x, y)
    #############################################################################
...
def normalize_columns(x):
    ...
    mu = (x.sum(axis=0) / x.shape[0]).view(1, -1)
    sigma = (((x - mu) ** 2).sum(axis=0) / (x.shape[0] - 1)).sqrt().view(1, -1)
    y = (x - mu) / sigma
